...
for RUN_ID in ${PR_RUN_IDS//,/ }; do
  ...
  # Download the artifacts of this run, if exist. A run can post more than one
  # notification, one per artifact named 'pr-notification-<key>', which is
  # downloaded into a directory of its own, as is any with a pattern. The
  # pattern also matches 'pr-notification' from an earlier version of the CI.
  gh run download ${RUN_ID} --pattern 'pr-notification*' --dir ${RUN_DIR} &> /dev/null || true

  for ARTIFACTS_DIR in ${RUN_DIR}/*; do
    # Move on if no notification is required, including when the pattern matched
    # nothing, and the loop runs with the pattern itself
    if [ ! -f ${ARTIFACTS_DIR}/pr-number.txt ]; then
      continue
    fi

    PR_NUMBER=$(cat ${ARTIFACTS_DIR}/pr-number.txt)
    ARTIFACT_NAME=$(basename ${ARTIFACTS_DIR})
    echo "@@@ Run ${RUN_ID}: notifying PR #${PR_NUMBER} of '${ARTIFACT_NAME}'"
    ...
    for f in body.txt key.txt hist.txt; do
      if [ ! -f ${ARTIFACTS_DIR}/${f} ]; then
        ...
        continue 2  # Note: continues the enclosing 'ARTIFACTS_DIR' loop
      fi
    done
...
NEXTHIST_TEMPLATE
    ...
    # '${ARTIFACT_NAME}' for a single run, as the artifacts endpoint lists
    # artifacts across all run attempts, and a re-run uploads a new one while
    # keeping the previous attempt's artifact.
    ARTIFACT_IDS=$(gh api "repos/{owner}/{repo}/actions/runs/${RUN_ID}/artifacts?name=${ARTIFACT_NAME}" --jq '.artifacts[].id')
