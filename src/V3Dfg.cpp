...
std::unique_ptr<DfgGraph> DfgGraph::clone() const {
    ...
    for (const DfgVertexAst& vtx : m_astVertices) {
        switch (vtx.type()) {
        case VDfgType::AstRd: {
...
DfgVertexVar* DfgVertex::getResultVar() {
    ...
    this->foreachSink([&resp](DfgVertex& sink) {
        ...
        const bool resHasModRdRef
            = resp->foreachSink([](const DfgVertex& dst) { return dst.is<DfgAstRd>(); });
        const bool varHasModRdRef
            = varp->foreachSink([](const DfgVertex& dst) { return dst.is<DfgAstRd>(); });
        if (resHasModRdRef != varHasModRdRef) {
            if (!resHasModRdRef) resp = varp;
            return false;
        }
