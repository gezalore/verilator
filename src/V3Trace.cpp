    ...
    void graphSimplify(bool initial) {
        if (initial) {
            ...
            for (auto it = vertices.begin(); it != vertices.end();) {
                ...
                if (TraceVarVertex* const vvertexp = vtx.cast<TraceVarVertex>()) {
            ...
            for (auto it = vertices.begin(); it != vertices.end();) {
                V3GraphVertex& vtx = *it++;
                if (TraceCFuncVertex* const vvertexp = vtx.cast<TraceCFuncVertex>()) {
                    vvertexp->rerouteEdges(&m_graph);
                    vvertexp->unlinkDelete(&m_graph);
                }
            }
        ...
        for (V3GraphVertex& vtx : m_graph.vertices()) {
            if (TraceTraceVertex* const vvertexp = vtx.cast<TraceTraceVertex>()) {
                ...
                if (alwaysEdgep) {
                    auto& inEdges = vvertexp->inEdges();
                    for (auto it = inEdges.begin(); it != inEdges.end();) {
                        V3GraphEdge& edge = *it++;
                        if (&edge != alwaysEdgep) edge.unlinkDelete();
                    }
                }
            }
        }
        ...
        for (auto it = vertices.begin(); it != vertices.end();) {
            V3GraphVertex& vtx = *it++;
            if (TraceActivityVertex* const vtxp = vtx.cast<TraceActivityVertex>()) {
                // Leave in the always vertex for later use.
                if (vtxp != m_alwaysVtxp && vtxp->outEmpty()) {
                    VL_DO_DANGLING(vtxp->unlinkDelete(&m_graph), vtxp);
                }
            }
        }
    }
