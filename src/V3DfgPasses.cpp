...
void V3DfgPasses::cse(DfgGraph& dfg, V3DfgCseContext& ctx) {
    // Remove common sub-expressions
    {
        ...
        auto& constVertices = dfg.constVertices();
        for (auto it = constVertices.begin(); it != constVertices.end();) {
            DfgConst& vtx = *it++;
            ...
            vtx.user<V3Hash>() = vtx.num().toHash() + varHash;
        }

        // Combine operation vertices
        auto& opVertices = dfg.opVertices();
        for (auto it = opVertices.begin(); it != opVertices.end();) {
            DfgVertex& vtx = *it++;
...
void V3DfgPasses::removeUnused(DfgGraph& dfg) {
    ...
    for (auto it = constVertices.begin(); it != constVertices.end();) {
        DfgConst& vtx = *it++;
        if (!vtx.hasSinks()) vtx.unlinkDelete(dfg);
    }
}
