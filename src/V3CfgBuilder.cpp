...
class CfgBuilder final : VNVisitorConst {
    ...
    void addTakenEdge(BasicBlock& src, BasicBlock& dst) {
        UASSERT(src.outEmpty(), "Taken edge should be added first");
    ...
    void addUntknEdge(BasicBlock& src, BasicBlock& dst) {
        UASSERT(src.outSize1(), "Untaken edge shold be added second");
        UASSERT(src.outEdges().frontp()->top() != &dst,
                "Untaken branch targets the same block as the taken branch");
        new ControlFlowEdge{m_cfgp.get(), &src, &dst};
    }

    // Add the given statement to the current BasicBlock
    void addStmt(AstNodeStmt* nodep) {
        const_cast<std::vector<AstNodeStmt*>&>(m_currBBp->m_stmtps).emplace_back(nodep);
    }
    ...
    CfgBuilder(const AstNodeProcedure* nodep) {
        ...
        const_cast<BasicBlock*&>(m_cfgp->m_enterp) = m_currBBp;
        // Visit each statement to build the control flow graph
        iterateAndNextConstNull(nodep->stmtsp());
        if (!m_cfgp) return;
        // The final block is the exit block
        const_cast<BasicBlock*&>(m_cfgp->m_exitp) = m_currBBp;
        ...
        while (m_cfgp->enter().stmtps().empty() && m_cfgp->enter().outSize1()) {
            ...
            const_cast<BasicBlock*&>(m_cfgp->m_enterp) = succp;
        }
        ...
        while (m_cfgp->exit().stmtps().empty() && m_cfgp->exit().inSize1()) {
            ...
            const_cast<BasicBlock*&>(m_cfgp->m_exitp) = prep;
        }

        // Compute reverse post-order enumeration and sort blocks, assign IDs
        {
            // Simple recursive algorith will do ...
            std::vector<const BasicBlock*> postOrderEnumeration;
            std::unordered_set<const BasicBlock*> visited;
            const std::function<void(const BasicBlock&)> visit = [&](const BasicBlock& bb) {
                // Mark and skip if already visited
                if (!visited.emplace(&bb).second) return;
                // Visit successors
                if (const BasicBlock* const succp = bb.takenSuccessorp()) visit(*succp);
                if (const BasicBlock* const succp = bb.untknSuccessorp()) visit(*succp);
        ...
        // Assign IDs to edges
        {
            ...
            // Set size in graph
            const_cast<size_t&>(m_cfgp->m_nEdges) = nEdges;
        }
