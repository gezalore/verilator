...
static const AstNodeModule* getModp(const AstNode* nodep) {
    const AstNodeModule* const modp = VN_CAST_CONST(nodep->user4p(), NodeModule);
    UASSERT_OBJ(modp, nodep, "Missing module pointer " << nodep);
    return modp;
}
...
class EmitCGatherDependencies final : AstNVisitor {
    ...
    virtual void visit(AstNodeVarRef* nodep) override {
        // All module variables should have a self pointer, and function locals should not
        if (!nodep->selfPointer().empty()
            && !EmitCBaseVisitor::isConstPoolMod(getModp(nodep->varp()))) {
    ...
    explicit EmitCGatherDependencies(AstCFunc* cfuncp) {
        ...
        addModDependency(getModp(cfuncp));
        iterate(cfuncp);
    }
...
class EmitCImp final : EmitCFunc {
    ...
    V3UniqueNameMap m_uniqueNameMap;  // For generating unique file names
    ...
    void openNextOutputFile(const std::set<string>& headers, const string& subFileName) {
        ...
        if (v3Global.opt.lintOnly()) {
            ...
            if (!subFileName.empty()) {
                ...
                filename = m_uniqueNameMap(filename);
            }
    ...
    void emitCommonImp(const AstNodeModule* modp) {
        ...
        // Emit common implementation contents for given AstNodeModule
        const auto emit = [this](const AstNodeModule* modp) {
            if (m_slow) {
        ...
        // Predicate to check if we actually need to emit anything. Used to avoid creating empty
        // output files.
        const auto nonEmptyOutput = [this](const AstNodeModule* modp) -> bool {
        ...
        if (nonEmptyOutput(modp) || nonEmptyOutput(classp)) {
