...
class EmitCImp final : EmitCFunc {
    // MEMBERS
    const AstNodeModule* const m_fileModp = nullptr;  // Files names constructed using this module
    ...
    void openNextOutputFile() {
        ...
        if (const int filenum = splitFilenumInc()) filenameNoExt += "__" + cvtToStr(filenum);
        if (m_slow) filenameNoExt += "__Slow";
        if (v3Global.opt.lintOnly()) {
            ...
            const string filename = filenameNoExt + ".cpp";
            newCFile(filename, /* slow: */ m_slow, /* source: */ true);
            m_ofp = v3Global.opt.systemC() ? new V3OutScFile(filename) : new V3OutCFile(filename);
        }

        ofp()->putsHeader();
        puts("// DESCRIPTION: Verilator output: Design internal implementation\n");
    ...
    void emitCoverageImp(const AstNodeModule*) {
        if (v3Global.opt.coverage()) {
            puts("\n// Coverage\n");
...
public:
    static void main(const AstNodeModule* modp, bool slow) { EmitCImp emitCImp(modp, slow); }
};
...
public:
    static void main(AstNodeModule* modp, bool slow) { EmitCTrace emitCTrace(modp, slow); }
};
...
void V3EmitC::emitcImp() {
    ...
    for (const AstNodeModule* nodep = v3Global.rootp()->modulesp(); nodep;
        ...
        if (VN_IS(nodep, Class)) continue;  // Implemented with ClassPackage
        EmitCImp::main(nodep, /* slow: */ true);
        EmitCImp::main(nodep, /* slow: */ false);
    }
