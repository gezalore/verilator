...
void DfgGraph::forEachVertex(std::function<void(DfgVertex&)> f) {
    for (auto it = m_varVertices.begin(); it != m_varVertices.end();) f(*it++);
    for (auto it = m_constVertices.begin(); it != m_constVertices.end();) f(*it++);
    for (auto it = m_opVertices.begin(); it != m_opVertices.end();) f(*it++);
}
