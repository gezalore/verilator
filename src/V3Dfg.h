...
public:
    ...
    uint32_t size() const {
        if (VN_IS(dtypep(), BasicDType)) return dtypep()->width();
...
public:
    ...
    static AstNodeDType* dtypeArray(AstNodeDType* subDtypep, uint32_t size) {
        UASSERT(isSupported(subDtypep), "Unsupported element type");
    ...
    static AstNodeDType* toDfgDType(const AstNodeDType* dtypep) {
        ...
        UASSERT(isSupported(dtypep), "Unsupported dtype");
...
DfgVertex* DfgVertexSplice::wholep() const {
    ...
    if (driverIsUnresolved(0)) return nullptr;
