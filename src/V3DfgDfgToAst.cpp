    ...
    void convertDriver(std::vector<Assignment>& assignments, FileLine* flp, AstNodeExpr* lhsp,
        ...
        if (DfgSplicePacked* const sPackedp = driverp->cast<DfgSplicePacked>()) {
            ...
            sPackedp->forEachSourceEdge([&](const DfgEdge& edge, size_t i) {
                ...
                UASSERT_OBJ(srcp, sPackedp, "Should have removed undriven sources");
                UASSERT_OBJ(!sPackedp->driverIsUnresolved(i), sPackedp,
                            "Unresolved driver should not reach DfgToAst");
                if (!sPackedp->driverIsDefault(i)) return;
                convertDriver(m_defaults, sPackedp->driverFileLine(i), lhsp, srcp);
            });
        ...
        if (DfgSpliceArray* const sArrayp = driverp->cast<DfgSpliceArray>()) {
            ...
            sArrayp->forEachSourceEdge([&](const DfgEdge& edge, size_t i) {
                ...
                // Convert source
                convertDriver(assignments, dflp, nLhsp, edge.sourcep());
