...
bool ControlFlowGraph::containsLoop() const {
    ...
    for (const V3GraphVertex& vtx : vertices()) {
        const BasicBlock& curr = static_cast<const BasicBlock&>(vtx);
        // For each successor of 'curr'
        for (const V3GraphEdge& edge : curr.outEdges()) {
            const BasicBlock& succ = *static_cast<const BasicBlock*>(edge.top());
            // IDs are the reverse post-order numbering, so easy to check for a back-edge
            if (succ.id() < curr.id()) return true;
        }
    }
    return false;
}
...
std::string ControlFlowEdge::dotLabel() const {
    ...
    const BasicBlock& src = *fromp()->as<BasicBlock>();
    const ControlFlowEdge* const untknp = src.untknEdgep();
    if (this == untknp) {
        label += " / F";
    } else if (untknp) {
        label += " / T";
