...
#define OPTIMIZATION_CHECK_ENABLED(id, name) \
    m_enabled[id] = !V3Os::getenvLong("VERILATOR_DFG_PEEPHOLE_DISABLE__" name, 0);
    FOR_EACH_DFG_PEEPHOLE_OPTIMIZATION(OPTIMIZATION_CHECK_ENABLED)
#undef OPTIMIZATION_CHECK_ENABLED
}

V3DfgPeepholeContext::~V3DfgPeepholeContext() {
    const auto emitStat = [this](int id, const char* name) {
        string str{name};
        std::transform(str.begin(), str.end(), str.begin(), [](unsigned char c) {  //
            return c == '_' ? ' ' : std::tolower(c);
        });
        V3Stats::addStat(string{"Optimizations, DFG Peephole, "} + str, m_count[id]);
    };
#define OPTIMIZATION_EMIT_STATS(id, name) emitStat(id, name);
    FOR_EACH_DFG_PEEPHOLE_OPTIMIZATION(OPTIMIZATION_EMIT_STATS)
#undef OPTIMIZATION_EMIT_STATS
}

class V3DfgPeephole final : public DfgVVisitor {
...
#define APPLYING(id) \
    if (VL_LIKELY(m_ctx.m_enabled[V3DfgPeepholeContext::id] \
                  && (++m_ctx.m_count[V3DfgPeepholeContext::id], m_changed = true, true)))

    // METHODS

    // Create a new 32-bit DfgConst vertex
    DfgConst* makeU32(FileLine* flp, uint32_t value) { return new DfgConst{m_dfg, flp, value}; }
    ...
    DfgConst* makeZero(FileLine* flp, uint32_t width) {
        ...
        return new DfgConst{m_dfg, constp};
    }
    ...
    bool tryPushBitwiseOpThroughConcat(Vertex* vtxp, DfgConst* constp, DfgConcat* concatp) {
        ...
        if (concatp->lhsp()->is<DfgConst>() || concatp->rhsp()->is<DfgConst>()  //
            ...
            APPLYING(PUSH_BITWISE_OP_THROUGH_CONCAT) {
                ...
                // The new Lhs vertex
                Vertex* const newLhsp = new Vertex{m_dfg, flp, lWidth};
                ...
                // The new Rhs vertex
                Vertex* const newRhsp = new Vertex{m_dfg, flp, rWidth};
                ...
                DfgConcat* const newConcat
                    = new DfgConcat{m_dfg, concatp->fileline(), concatp->width()};
    ...
    bool tryPushCompareOpThroughConcat(Vertex* vtxp, DfgConst* constp, DfgConcat* concatp) {
        ...
        if (concatp->lhsp()->is<DfgConst>() || concatp->rhsp()->is<DfgConst>()) {
            APPLYING(PUSH_COMPARE_OP_THROUGH_CONCAT) {
                ...
                // The new Lhs vertex
                Vertex* const newLhsp = new Vertex{m_dfg, flp, 1};
                DfgConst* const newLhsConstp = makeZero(constp->fileline(), lWidth);
                newLhsConstp->num().opSel(constp->num(), width - 1, rWidth);
                newLhsp->lhsp(newLhsConstp);
                newLhsp->rhsp(concatp->lhsp());
                ...
                // The new Rhs vertex
                Vertex* const newRhsp = new Vertex{m_dfg, flp, 1};
                DfgConst* const newRhsConstp = makeZero(constp->fileline(), rWidth);
                newRhsConstp->num().opSel(constp->num(), rWidth - 1, 0);
                newRhsp->lhsp(newRhsConstp);
                newRhsp->rhsp(concatp->rhsp());
                ...
                DfgVertexWithArity<2>* const replacementp
                    = std::is_same<Vertex, DfgEq>::value
                          ? new DfgAnd{m_dfg, concatp->fileline(), 1}
                          : nullptr;
    ...
    static bool concatTreeWithAtLeastOneWidth1Leaf(const DfgVertex* vtxp) {
        if (vtxp->width() == 1) return true;
        if (const DfgConcat* const concatp = vtxp->cast<DfgConcat>()) {
            return concatTreeWithAtLeastOneWidth1Leaf(concatp->lhsp())
                   || concatTreeWithAtLeastOneWidth1Leaf(concatp->rhsp());
        }
        return false;
    }
    ...
    void optimizeReduction(Vertex* vtxp) {
        ...
        if (DfgConcat* const concatp = srcp->cast<DfgConcat>()) {
            if (concatTreeWithAtLeastOneWidth1Leaf(concatp)) {
                APPLYING(REDUCTION_OF_ONE_BIT_CONCAT) {
                    // The new Lhs vertex
                    Vertex* const newLhsp = new Vertex{m_dfg, flp, 1};
                    newLhsp->srcp(concatp->lhsp());

                    // The new Rhs vertex
                    Vertex* const newRhsp = new Vertex{m_dfg, flp, 1};
                    newRhsp->srcp(concatp->rhsp());

                    // The replacement Vertex
                    FileLine* const cflp = concatp->fileline();
                    DfgVertexWithArity<2>* replacementp = nullptr;
                    if /* constexpr */ (std::is_same<Vertex, DfgRedAnd>::value) {
                        replacementp = new DfgAnd{m_dfg, cflp, 1};
                    } else if /* constexpr */ (std::is_same<Vertex, DfgRedOr>::value) {
                        replacementp = new DfgOr{m_dfg, cflp, 1};
                    } else {
                        replacementp = new DfgXor{m_dfg, cflp, 1};
                    }
                    replacementp->relinkSource<0>(newLhsp);
                    replacementp->relinkSource<1>(newRhsp);
        ...
        if (DfgCond* const condp = srcp->cast<DfgCond>()) {
            if (condp->thenp()->is<DfgConst>() || condp->elsep()->is<DfgConst>()) {
                APPLYING(PUSH_REDUCTION_THROUGH_COND_WITH_CONST_BRANCH) {
                    // The new 'then' vertex
                    Vertex* const newThenp = new Vertex{m_dfg, vtxp->fileline(), 1};
                    newThenp->srcp(condp->thenp());

                    // The new 'else' vertex
                    Vertex* const newElsep = new Vertex{m_dfg, vtxp->fileline(), 1};
                    newElsep->srcp(condp->elsep());

                    // The replacement Cond vertex
                    DfgCond* const newCondp = new DfgCond{m_dfg, condp->fileline(), 1};
    ...
    void visit(DfgExtend* vtxp) override {
        ...
        APPLYING(REPLACE_EXTEND) {
            DfgConcat* const replacementp = new DfgConcat{m_dfg, flp, vtxp->width()};
    ...
    void visit(DfgNot* vtxp) override {
        ...
        if (DfgCond* const condp = vtxp->srcp()->cast<DfgCond>()) {
            ...
            if (condp->thenp()->is<DfgConst>() || condp->elsep()->is<DfgConst>()) {
                APPLYING(PUSH_NOT_THROUGH_CONCAT) {
                    // The new 'then' vertex
                    DfgNot* const newThenp = new DfgNot{m_dfg, vtxp->fileline(), vtxp->width()};
                    newThenp->srcp(condp->thenp());

                    // The new 'else' vertex
                    DfgNot* const newElsep = new DfgNot{m_dfg, vtxp->fileline(), vtxp->width()};
                    newElsep->srcp(condp->elsep());

                    // The replacement Cond vertex
                    DfgCond* const newCondp = new DfgCond{m_dfg, condp->fileline(), vtxp->width()};
                    newCondp->condp(condp->condp());
                    newCondp->thenp(newThenp);
                    newCondp->elsep(newElsep);
        ...
        if (DfgNeq* const neqp = vtxp->srcp()->cast<DfgNeq>()) {
            APPLYING(REPLACE_NOT_NEQ) {
                DfgEq* const replacementp = new DfgEq{m_dfg, neqp->fileline(), vtxp->width()};
    ...
    void visit(DfgAnd* vtxp) override {
        ...
        if (lhsp->is<DfgNot>() && rhsp->is<DfgNot>()) {
            APPLYING(REPLACE_AND_OF_NOT_AND_NOT) {
                DfgOr* const orp = new DfgOr{m_dfg, flp, vtxp->width()};
                orp->lhsp(lhsp->as<DfgNot>()->srcp());
                orp->rhsp(rhsp->as<DfgNot>()->srcp());
                DfgNot* const notp = new DfgNot{m_dfg, flp, vtxp->width()};
                notp->srcp(orp);
        ...
        if (DfgNot* const lhsNotp = lhsp->cast<DfgNot>()) {
            if (DfgNot* const rhsNotp = rhsp->cast<DfgNot>()) {
                APPLYING(REPLACE_OR_OF_NOT_AND_NOT) {
                    DfgAnd* const andp = new DfgAnd{m_dfg, flp, vtxp->width()};
            ...
            if (DfgNeq* const rhsNeqp = rhsp->cast<DfgNeq>()) {
                APPLYING(REPLACE_OR_OF_NOT_AND_NEQ) {
                    DfgAnd* const andp = new DfgAnd{m_dfg, flp, vtxp->width()};
                    andp->lhsp(lhsNotp->srcp());
                    DfgEq* const newRhsp = new DfgEq{m_dfg, rhsp->fileline(), rhsp->width()};
                    ...
                    if (lhsConcatp->lhsp()->isZero() && rhsConcatp->rhsp()->isZero()) {
                        APPLYING(REPLACE_OR_OF_CONCAT_ZERO_LHS_AND_CONCAT_RHS_ZERO) {
                            DfgConcat* const replacementp
                                = new DfgConcat{m_dfg, flp, vtxp->width()};
    ...
    void visit(DfgAdd* vtxp) override {
        ...
        // Predicate for zero extending Concat
        const auto isZExt = [](const DfgVertex& vtx) -> bool {
            if (const DfgConcat* const concatp = vtx.cast<DfgConcat>()) {
                return concatp->lhsp()->isZero();
            }
            return false;
        };

        const bool onlyZExtSinks = !vtxp->findSink<DfgVertex>([&](const DfgVertex& sink) {  //
            return !isZExt(sink);
        });

        const std::function<bool(const DfgVertex&)> isZExtRecursive
            = [&](const DfgVertex& vtx) -> bool {
            if (const DfgAdd* const addp = vtx.cast<DfgAdd>()) {
                return isZExtRecursive(*addp->lhsp()) && isZExtRecursive(*addp->rhsp());
            }
            return isZExt(vtx);
        };

        const bool onlyZExtSources = isZExtRecursive(*vtxp);

        if (onlyZExtSinks && onlyZExtSources && vtxp->width() < VL_QUADSIZE) {
            APPLYING(WIDEN_ADD_OPERAND) {
                const auto widenOperand = [this](DfgVertex& vtx, uint32_t width) {
                    FileLine* const flp = vtx.fileline();
                    UASSERT_OBJ(width > vtx.width(), flp, "widening does not widen");
                    const uint32_t extension = width - vtx.width();
                    DfgConcat* const newConcatp = new DfgConcat{m_dfg, flp, width};
                    newConcatp->lhsp(makeZero(vtx.fileline(), extension));
                    newConcatp->rhsp(&vtx);
                    return newConcatp;
                };

                vtxp->forEachSink([&](DfgVertex& sink) {
                    // For now, new node for each sink. Next round of CSE will merge them if
                    // possible.
                    const uint32_t width = sink.width();
                    // Don't widen past a machine word size
                    if (width > VL_QUADSIZE) return;
                    DfgAdd* const replacementp = new DfgAdd{m_dfg, vtxp->fileline(), width};
                    replacementp->lhsp(widenOperand(*vtxp->lhsp(), width));
                    replacementp->rhsp(widenOperand(*vtxp->rhsp(), width));
                    sink.replaceWith(replacementp);
                    m_changed = true;
                });
        ...
        if (DfgConst* const rConstp = rhsp->cast<DfgConst>()) {
            ...
            if (vtxp->width() == 1 && rConstp->toU32() == 1) {
                APPLYING(REPLACE_SUB_WITH_NOT) {
                    DfgNot* const replacementp = new DfgNot{m_dfg, vtxp->fileline(), 1};
        ...
        if (DfgConcat* const concatp = fromp->cast<DfgConcat>()) {
            ...
            if (msb < rhsp->width()) {
                ...
                APPLYING(REMOVE_SEL_FROM_LHS_OF_CONCAT) {
                    vtxp->fromp(lhsp);
                    vtxp->lsbp(makeU32(flp, lsb - rhsp->width()));
                }
                ...
                APPLYING(PUSH_SEL_THROUGH_CONCAT) {
                    ...
                    // The new Lhs vertex
                    DfgSel* const newLhsp = new DfgSel{m_dfg, flp, lSelWidth};
                    newLhsp->fromp(lhsp);
                    newLhsp->lsbp(makeU32(lsbp->fileline(), 0));
                    newLhsp->widthp(makeU32(widthp->fileline(), lSelWidth));

                    // The new Rhs vertex
                    DfgSel* const newRhsp = new DfgSel{m_dfg, flp, rSelWidth};
                    newRhsp->fromp(rhsp);
                    newRhsp->lsbp(makeU32(lsbp->fileline(), lsb));
                    newRhsp->widthp(makeU32(widthp->fileline(), rSelWidth));

                    // The replacement Concat vertex
                    DfgConcat* const newConcat
                        = new DfgConcat{m_dfg, concatp->fileline(), vtxp->width()};
                    newConcat->lhsp(newLhsp);
                    newConcat->rhsp(newRhsp);
            ...
            if (!notp->hasMultipleSinks()) {
                ...
                APPLYING(PUSH_SEL_THROUGH_NOT) {
                    ...
                    DfgNot* const replacementp
                        = new DfgNot{m_dfg, notp->fileline(), vtxp->width()};
                    vtxp->replaceWith(replacementp);
        ...
        if (DfgSel* const selp = fromp->cast<DfgSel>()) {
            ...
            if (DfgConst* const sourceLsbp = selp->lsbp()->cast<DfgConst>()) {
                ...
                APPLYING(REPLACE_SEL_FROM_SEL) {
                    ...
                    // Adjust LSB
                    vtxp->lsbp(makeU32(flp, lsb + sourceLsbp->toU32()));
                }
            }
        }
        ...
        if (DfgCond* const condp = fromp->cast<DfgCond>()) {
            ...
            if (condp->thenp()->is<DfgConst>() || condp->elsep()->is<DfgConst>()) {
                APPLYING(PUSH_SEL_THROUGH_COND) {
                    // The new 'then' vertex
                    DfgSel* const newThenp = new DfgSel{m_dfg, flp, vtxp->width()};
                    newThenp->fromp(condp->thenp());
                    newThenp->lsbp(makeU32(lsbp->fileline(), lsb));
                    newThenp->widthp(makeU32(widthp->fileline(), width));

                    // The new 'else' vertex
                    DfgSel* const newElsep = new DfgSel{m_dfg, flp, vtxp->width()};
                    newElsep->fromp(condp->elsep());
                    newElsep->lsbp(makeU32(lsbp->fileline(), lsb));
                    newElsep->widthp(makeU32(widthp->fileline(), width));
            ...
            if (lsb == 0) {
                ...
                APPLYING(PUSH_SEL_THROUGH_SHIFTL) {
                    ...
                    DfgShiftL* const newShiftLp
                        = new DfgShiftL{m_dfg, shiftLp->fileline(), vtxp->width()};
                            ...
                            if (vtxp->width() == rSelp->fromp()->width()
                                ...
                                APPLYING(REPLACE_CONCAT_ZERO_AND_SEL_TOP_WITH_SHIFTR) {
                                    DfgShiftR* const replacementp
                                        = new DfgShiftR{m_dfg, flp, vtxp->width()};
                                    replacementp->lhsp(rSelp->fromp());
                                    replacementp->rhsp(makeU32(flp, lConstp->width()));
                                    vtxp->replaceWith(replacementp);
                                    return;
                                }
                            }
                            ...
                            if (vtxp->width() == lSelp->fromp()->width()
                                ...
                                APPLYING(REPLACE_CONCAT_SEL_BOTTOM_AND_ZERO_WITH_SHIFTL) {
                                    DfgShiftL* const replacementp
                                        = new DfgShiftL{m_dfg, flp, vtxp->width()};
                                    replacementp->lhsp(lSelp->fromp());
                                    replacementp->rhsp(makeU32(flp, rConstp->width()));
                                    vtxp->replaceWith(replacementp);
                                    return;
                                }
                            }
...

        {
            ...
            if (lNot && rNot) {
                APPLYING(PUSH_CONCAT_THROUGH_NOTS) {
                    vtxp->lhsp(lNot->srcp());
                    vtxp->rhsp(rNot->srcp());
                    DfgNot* const replacementp = new DfgNot{m_dfg, flp, vtxp->width()};
                    vtxp->replaceWith(replacementp);
                    replacementp->srcp(vtxp);
                    return;
                }
                ...
                if (lLsbp && lWidthp && rLsbp && rWidthp) {
                    if (lSelp->fromp()->equals(*rSelp->fromp())) {
                        if (lLsbp->toU32() == rLsbp->toU32() + rWidthp->toU32()) {
                            ...
                            DfgSel* const joinedSelp = new DfgSel{m_dfg, flp, width};
                            joinedSelp->fromp(rSelp->fromp());
                            joinedSelp->lsbp(rSelp->lsbp());
                            joinedSelp->widthp(makeU32(flp, width));
                            return joinedSelp;
                        }
                    }
                }
    ...
    void visit(DfgCond* vtxp) override {
        ...
        if (DfgNeq* const condNeqp = condp->cast<DfgNeq>()) {
            APPLYING(SWAP_COND_WITH_NEQ_CONDITION) {
                DfgEq* const newCondp = new DfgEq{m_dfg, condp->fileline(), condp->width()};
        ...
        if (vtxp->width() == 1) {
            if (thenp->isZero()) {  // a ? 0 : b becomes ~a & b
                APPLYING(REPLACE_COND_WITH_THEN_BRANCH_ZERO) {
                    DfgAnd* const repalcementp = new DfgAnd{m_dfg, flp, 1};
                    DfgNot* const notp = new DfgNot{m_dfg, flp, 1};
                    notp->srcp(condp);
            ...
            if (thenp->isOnes()) {  // a ? 1 : b becomes a | b
                APPLYING(REPLACE_COND_WITH_THEN_BRANCH_ONES) {
                    DfgOr* const repalcementp = new DfgOr{m_dfg, flp, 1};
...
#undef APPLYING
    ...
    void processVertex(DfgVertex& vtx) {
        // If it's a DfgVar with no consumers, we can remove it if it's not externally
        // referenced, or if it's not driven here (i.e.: a read reference of an externally
        // referenced variable)
        if (DfgVar* varVtxp = vtx.cast<DfgVar>()) {
            const bool externallyReferencedDrivenHere
                = varVtxp->hasExtRefs() && varVtxp->driverp();
            if (!varVtxp->hasSinks() && !externallyReferencedDrivenHere) {
                // Also delete the referenced AstVar if it's not externally referenced
                if (!varVtxp->hasExtRefs()) {
                    varVtxp->varp()->unlinkFrBack();
                    varVtxp->varp()->deleteTree();
                }
                // Unlink and delete vertex
                varVtxp->unlinkDelete(m_dfg);
                m_changed = true;
            }
            // Nothing to transform on a DfgVar, moving on
            return;
        }
        ...
        if (!vtx.hasSinks()) {
            UASSERT_OBJ(m_changed, &vtx, "Transforming vertex should have set m_changed");
            vtx.unlinkDelete(m_dfg);
        }
    }
    ...
    void runToFixedPoint() {
        while (true) {
            ...
            m_changed = false;
            m_dfg.forEachVertex([this](DfgVertex& vtx) { processVertex(vtx); });
            if (!m_changed) break;
            ...
            m_changed = false;
            m_dfg.forEachVertexBackwards([this](DfgVertex& vtx) { processVertex(vtx); });
            if (!m_changed) break;
        }
    }
...
public:
    static void apply(DfgGraph& dfg, V3DfgPeepholeContext& ctx) {
        V3DfgPeephole{dfg, ctx}.runToFixedPoint();
    }
};
