...
void V3DfgPasses::synthesizeCyclicAlways(DfgGraph& dfg, V3DfgContext& ctx) {
    ...
    std::vector<DfgAlways*> circularAlwaysps;
    {
        // Find cycles
        const auto userDataInUse = dfg.userDataInUse();
        const uint32_t numNonTrivialSCCs = ColorStronglyConnectedComponents::apply(dfg);
        ...
        for (DfgVertex& vtx : dfg.opVertices()) {
            ...
            if (!alwaysp) continue;
            circularAlwaysps.emplace_back(alwaysp);
        }
    }
    ...
    for (DfgAlways* const alwaysp : circularAlwaysps) {
