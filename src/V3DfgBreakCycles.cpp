...
public:
    SccInfo(DfgGraph& dfg)
        : m_dfg{dfg} {
        // Initialize m_vtx2Scc for the graph
        ...
        m_dfg.forEachVertex([&](const DfgVertex& vtx) {
            m_vtx2Scc[&vtx] = vtx2Scc[vtx];
            if (vtx2Scc[vtx]) ++m_nCyclicVertices;
        });
    }
    ...
    void updateAcyclic(DfgVertex& vtx) {
        // Mark as acyclic
        ...
        vtx.foreachSink([&](DfgVertex& dst) {
            ...
            // If the sink only reads non-cyclic vertices, it has also become acyclic
            const bool stillCyclic = dst.foreachSource([&](const DfgVertex& src) {  //
                return m_vtx2Scc.at(&src);
            });
            if (!stillCyclic) updateAcyclic(dst);
            return false;
        });
        ...
        vtx.foreachSource([&](DfgVertex& src) {
            ...
            // If the source only drives non-cyclic vertices, it has also become acyclic
            const bool stillCyclic = src.foreachSink([&](const DfgVertex& dst) {  //
                return m_vtx2Scc.at(&dst);
            });
            if (!stillCyclic) updateAcyclic(src);
            return false;
        });
    }
    ...
    void fixUpPacked(DfgVertex& vtx) {
        ...
        if (!vtx.hasSinks()) {
            ...
            replacementp->foreachSink([&](DfgVertex& sink) {
                if (m_sccInfo.get(sink)) m_sccInfo.updateAcyclic(sink);
                return false;
            });
        }
    }
