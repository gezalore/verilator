...
public:
    ...
    inline void iterateChildren(T_Node* nodep);
    /// Call visit()s on nodep's children in backp() order
    template <typename T_Node>
    inline void iterateChildrenBackwards(T_Node* nodep);
    /// Call visit()s on const nodep's children
    template <typename T_Node>
    inline void iterateChildrenConst(T_Node* nodep);
...
// Tables generated by 'astgen'.
#include "V3Ast__gen_tables.h"
...
template <typename T_Node>
void VNVisitor::iterateChildren(T_Node* nodep) {
    static_assert(std::is_base_of<AstNode, T_Node>::value,
                  "'T_Node' must be a subtype of AstNode");
    static_assert(VNMaxOps<T_Node>::value != 0,
                  "'iterateChildren' called on node known to have no children");
    nodep->iterateChildren(*this);
}
template <typename T_Node>
void VNVisitor::iterateChildrenBackwards(T_Node* nodep) {
    static_assert(std::is_base_of<AstNode, T_Node>::value,
                  "'T_Node' must be a subtype of AstNode");
    static_assert(VNMaxOps<T_Node>::value != 0,
                  "'iterateChildrenBackwards' called on node known to have no children");
    nodep->iterateChildrenBackwards(*this);
}
template <typename T_Node>
void VNVisitor::iterateChildrenConst(T_Node* nodep) {
    static_assert(std::is_base_of<AstNode, T_Node>::value,
                  "'T_Node' must be a subtype of AstNode");
    static_assert(VNMaxOps<T_Node>::value != 0,
                  "'iterateChildrenConst' called on node known to have no children");
