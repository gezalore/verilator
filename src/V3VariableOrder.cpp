...
class VariableOrder final {
    ...
    struct VarAttributes {
        EmitCBaseVisitor::VarClass kind;  // Variable class (IO/Local/Temp/Param, etc)
        uint32_t stratum;  // Roughly word size
    ...
    void simpleSortVars(std::vector<AstVar*>& varps) {
        stable_sort(varps.begin(), varps.end(),
                    [this](const AstVar* ap, const AstVar* bp) -> bool {
                        ...
                        if (attrA.kind != attrB.kind) {  // First sort by class
                            return attrA.kind < attrB.kind;
                        }
    ...
    // Sort by MTask-affinity group first, then the same as simpleSortVars
    void tspSortVars(std::vector<AstVar*>& varps) {
        ...
        for (const V3TSP::TspStateBase* const stateBasep : sortedStates) {
            ...
            std::vector<AstVar*>& subVec = m2v[statep->mtaskIds()];
            simpleSortVars(subVec);
            for (AstVar* const varp : subVec) { varps.push_back(varp); }
            VL_DO_DANGLING(delete statep, statep);
        }

        // Finally add the variables with no known MTask affinity
        {
            std::vector<AstVar*>& subVec = m2v[MTaskIdSet()];
            simpleSortVars(subVec);
            for (AstVar* const varp : subVec) { varps.push_back(varp); }
        }
    }
    ...
    void orderModuleVars(AstNodeModule* modp) {
        ...
        for (AstNode *nodep = modp->stmtsp(), *nextp; nodep; nodep = nextp) {
            ...
            if (AstVar* const varp = VN_CAST(nodep, Var)) {
                ...
                // Variable class
                attributes.kind = EmitCBaseVisitor::varClass(varp);
                // Stratum
        ...
        if (!varps.empty()) {
            ...
            // the front of the list so they come out first in in dumps/XML.
            AstVar* const firstp = varps.front();
            for (AstVar* const varp : varps) {
                if (varp != firstp) { firstp->addNext(varp); }
            }
