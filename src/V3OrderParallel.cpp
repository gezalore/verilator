...
class SiblingMC final : public MergeCandidate {
    ...
    V3List2Links<SiblingMC> m_aLinks;  // List links to store instances of this class
    V3List2Links<SiblingMC> m_bLinks;  // List links to store instances of this class

public:
    // List type to store instances of this class
    using AList = V3List2<SiblingMC, &SiblingMC::m_aLinks>;
    using BList = V3List2<SiblingMC, &SiblingMC::m_bLinks>;
...
class Partitioner final {
    ...
    Partitioner(const OrderGraph& orderGraph, OrderMoveGraph& moveGraph)
        ...
        auto& vertices = m_mTaskGraphp->vertices();
        for (auto it = vertices.begin(); it != vertices.end();) {
            LogicMTask* const mtaskp = (*it++).as<LogicMTask>();
...
AstExecGraph* V3Order::createParallel(OrderGraph& orderGraph, const std::string& tag,
    ...
    for (auto it = vertices.begin(); it != vertices.end();) {
        OrderMoveVertex* const mVtxp = (*it++).as<OrderMoveVertex>();
        if (!mVtxp->userp()) {
    ...
    for (V3GraphVertex& vtx : moveGraphp->vertices()) {
        ...
        for (auto it = outEdges.begin(); it != outEdges.end();) {
            V3GraphEdge& edge = *it++;
            const OrderMoveVertex* const toMVtxp = edge.top()->as<OrderMoveVertex>();
            if (mtaskp != toMVtxp->userp()) edge.unlinkDelete();
        }
    }
    ...
    auto& remainingVertices = moveGraphp->vertices();
    for (auto it = remainingVertices.begin(); it != remainingVertices.end();) {
