...
// or the whole netlist (when T_Scoped is true), into a DfgGraph composed
// only of DfgVertexVar and DfgLogic vertices. The DfgLogic vertices can
// be synthesized into primitive operations later.
...
class AstToDfgVisitor final : public VNVisitor {
    ...
    void connect(DfgLogic& vtx, const std::vector<DfgVertexVar*>& iVarps,
        ...
        for (DfgVertexVar* const oVarp : oVarps) {
            FileLine* const flp = oVarp->fileline();
            AstNodeDType* const dtypep = oVarp->dtypep();
            if (oVarp->is<DfgVarPacked>()) {
                if (!oVarp->srcp()) oVarp->srcp(new DfgSplicePacked{m_dfg, flp, dtypep});
                oVarp->srcp()->as<DfgSplicePacked>()->addUnresolvedDriver(&vtx);
            } else if (oVarp->is<DfgVarArray>()) {
                if (!oVarp->srcp()) oVarp->srcp(new DfgSpliceArray{m_dfg, flp, dtypep});
                oVarp->srcp()->as<DfgSpliceArray>()->addUnresolvedDriver(&vtx);
            } else {
                vtx.v3fatalSrc("Unhandled DfgVertexVar sub-type");  // LCOV_EXCL_LINE
            }
        }
    }
