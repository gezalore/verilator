...
class BasicBlock final : public V3GraphVertex {
    ...
    // STATE - Immutable after construction, mutated by CfgBuilder
    // V3GraphEdge::user() is set to the unique id by CfgBuilder
    const std::vector<AstNodeStmt*> m_stmtps;  // statements in this BasicBlock
...
class ControlFlowGraph final : public V3Graph {
    ...
    // STATE - Immutable after construction, mutated by CfgBuilder
    BasicBlock* const m_enterp = nullptr;  // The singular entry vertex
    BasicBlock* const m_exitp = nullptr;  // The singular exit vertex
    const size_t m_nBasicBlocks = 0;  // Number of BasicBlocks in this ControlFlowGraph
    const size_t m_nEdges = 0;  // Number of ControlFlowEdges in this ControlFlowGraph
