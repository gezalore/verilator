...
private:
    ...
    virtual void visit(AstNodeVarRef* nodep) override {
        ...
        // In C++ mode, replace references to const variables initialized with a known value with
        // the initializer. Note that while in C++ one can assign to a const variable via
        // const_cast, code generated by Verilator does not do that, so any const variable is
        // equivalent to it's initializer.
        if (m_doCpp && nodep->access().isReadOnly() && nodep->varp()->isConst()
            && !nodep->isWide()) {
            if (const AstConst* const constp = VN_CAST(nodep->varp()->valuep(), Const)) {
                replaceNum(nodep, constp->num());
            }
        }
    }
