...
struct StmtProperties final {
    ...
    // Nothing should move across this statement, nor should it be merged
    bool m_isFence = false;
    // Side effect, but no model state change, nor does it observe model satet (e.g. $display)
    // Can merge, and move pure statements across it
    bool m_sideEffectNoModelState = false;
    // Side effect, that can read or write public state. Can merge if condition not public
    bool m_sideEffectPublicState = false;
    // Accesses public variables
    bool m_vrefPublic = false;
    AstNodeStmt* m_prevWithSameCondp = nullptr;  // Previous node in same list, with same condition
    bool writesConditionVar(bool condPublic) const {
        ...
        // If the condition contains a public variabel, then it might be written
        if (condPublic && m_sideEffectPublicState) return true;
...
class CodeMotionAnalysisVisitor final : public VNVisitorConst {
    ...
    void analyzeStmt(AstNodeStmt* nodep, bool tryCondMatch) {
        ...
        if (outerPropsp) {
            ...
            if (m_propsp->m_sideEffectNoModelState) outerPropsp->m_sideEffectNoModelState = true;
            if (m_propsp->m_sideEffectPublicState) outerPropsp->m_sideEffectPublicState = true;
            if (m_propsp->m_vrefPublic) outerPropsp->m_vrefPublic = true;
        }
    }
    ...
    void analyzeVarRef(AstVarRef* nodep) {
        ...
        if (varp->isSigPublic() || varp->isWrittenByDpi()) m_propsp->m_vrefPublic = true;
    }
    ...
    void checkProperties(AstNode* nodep) {
        ...
        // Call to a non pure DPI import
        if (const AstNodeCCall* const ccallp = VN_CAST(nodep, NodeCCall)) {
            AstCFunc* const funcp = ccallp->funcp();
            if (funcp->dpiImportWrapper() && !funcp->dpiPure()) {
                m_propsp->m_sideEffectPublicState = true;
                return;
            }
        }

        // The following we assume does not change or ovserve model state
        if (VN_IS(nodep, Display) || VN_IS(nodep, Stop)) {
            m_propsp->m_sideEffectNoModelState = true;
            return;
        }
...
class CodeMotionOptimizeVisitor final : public VNVisitor {
    ...
    bool areSwappable(const AstNodeStmt* ap, const AstNodeStmt* bp) const {
        ...
        // Don't move side effecting statements across each other, but can move other across them
        bool aSideEffect = aProps.m_sideEffectNoModelState || aProps.m_sideEffectPublicState;
        bool bSideEffect = bProps.m_sideEffectNoModelState || bProps.m_sideEffectPublicState;
        if (aSideEffect && bSideEffect) return false;
        // Don't move across if a public variable is involved and may be observed externally
        if (aProps.m_vrefPublic && bProps.m_sideEffectPublicState) return false;
        if (bProps.m_vrefPublic && aProps.m_sideEffectPublicState) return false;
...
class MergeCondVisitor final : public VNVisitor {
    ...
    bool m_mgCondPublic = false;  // True if m_mgCondp contains a public variable
    ...
    bool addToList(AstNodeStmt* nodep, AstNodeExpr* condp) {
        ...
        if (!m_mgFirstp) {
            ...
            condp->foreach([&](const AstVarRef* nodep) {
                AstVar* const varp = nodep->varp();
                varp->user1(1);
                if (varp->isSigPublic() || varp->isWrittenByDpi()) m_mgCondPublic = true;
            });
            ...
            if ((*m_stmtPropertiesp)(nodep).writesConditionVar(m_mgCondPublic)) {
            ...
            while (true) {
                ...
                if (props.m_isFence || props.writesConditionVar(m_mgCondPublic)) break;
    ...
    bool checkOrMakeMergeable(const AstNodeStmt* nodep) {
        ...
        if (m_mgFirstp && props.writesConditionVar(m_mgCondPublic)) mergeEnd();
