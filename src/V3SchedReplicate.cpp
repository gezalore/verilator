...
enum Region : uint8_t { INPUT = 0x1, ACTIVE = 0x2, NBA = 0x4 };

//##############################################################################
// Data structures (graph types)

class Vertex VL_NOT_FINAL : public V3GraphVertex {
    uint8_t m_drivingRegions = 0;  // The scheduling regions driving this vertex (var or logic)

public:
    Vertex(V3Graph* graphp)
        : V3GraphVertex{graphp} {}
    uint8_t drivingRegions() const { return m_drivingRegions; }
    void addDrivingRegions(uint8_t regions) { m_drivingRegions |= regions; }

    // For graph dumping
    string dotColor() const override {
        switch (m_drivingRegions) {
        case 0: return "black";
...
class LogicVertex final : public Vertex {
    ...
    Region const m_assignedRegion;  // The region this logic is originally assigned to
...
public:
    ...
    Region assignedRegion() const { return m_assignedRegion; }
...
std::unique_ptr<Graph> buildGraph(const LogicRegions& logicRegions) {
    ...
    const auto addLogic = [&](Region region, AstScope* scopep, AstActive* activep) {
        ...
        for (AstNode* nodep = activep->stmtsp(); nodep; nodep = nodep->nextp()) {
            LogicVertex* const lvtxp = new LogicVertex{graphp, scopep, senTreep, nodep, region};
            ...
            nodep->foreach<AstVarRef>([&](AstVarRef* refp) {
                ...
                // If read, add var -> logic edge TODO: explain the third term in the
                // condition
    ...
    logicRegions.m_pre.foreach ([&](AstScope* scopep, AstActive* activep) {  //
        addLogic(ACTIVE, scopep, activep);
    });
    logicRegions.m_active.foreach ([&](AstScope* scopep, AstActive* activep) {  //
        addLogic(ACTIVE, scopep, activep);
    });
    logicRegions.m_nba.foreach ([&](AstScope* scopep, AstActive* activep) {  //
        addLogic(NBA, scopep, activep);
    });

    return std::unique_ptr<Graph>{graphp};
}

void propagateDrivingRegions(Graph* graphp) {
    // TODO: better algorithm (the graph is guaranteed to be acyclic at this point,
