...
static void DELAY_LIST(AstNode* listp, AstDelay* delayp) {
    ...
    for (AstNode* nodep = listp; nodep; nodep = nodep->nextp()) {
        ...
        AstAssign* const assignp = VN_AS(alwaysp->stmtsp(), Assign);
        assignp->timingControlp(nodep == listp ? delayp : delayp->cloneTree(false));
    }
}
...
static void STRENGTH_LIST(AstNode* listp, AstStrengthSpec* specp) {
    ...
    for (AstNode* nodep = listp; nodep; nodep = nodep->nextp()) {
        ...
        AstAlways* const alwaysp = VN_AS(nodep, Always);
        AstAssign* const assignp = VN_AS(alwaysp->stmtsp(), Assign);
        assignp->strengthSpecp(nodep == listp ? specp : specp->cloneTree(false));
    }
}
...
assignOne<alwaysp>:
                variable_lvalue '=' expr                { $$ = AstAlways::newCAssign($2, $1, $3); }
        ;
...
netSig<varp>:                   // IEEE: net_decl_assignment -  one element from list_of_port_identifiers
        ...
        |       netId variable_dimensionListE sigAttrListE '=' expr
                        { $$ = VARDONEA($<fl>1, *$1, $2, $3);
                          ...
                          AstAlways* const alwaysp = AstAlways::newCAssign($4, new AstParseRef{$<fl>1, *$1}, $5, delayp);
                          if (GRAMMARP->m_netStrengthp) {
                              VN_AS(alwaysp->stmtsp(), Assign)->strengthSpecp(GRAMMARP->m_netStrengthp->cloneTree(false));
                          }
                          AstNode::addNext<AstNode, AstNode>($$, alwaysp); }
        ;
...
gateBuf<nodep>:
                gateFront variable_lvalue ',' exprList ')'
                        { AstNodeExpr* inp = $4;
                          ...
                          $$->addNext(AstAlways::newCAssign($<fl>1, $2, GRAMMARP->createGatePin(inp->cloneTree(false))));
                          for (AstNodeExpr* outp = $4; outp->nextp(); outp = VN_CAST(outp->nextp(), NodeExpr)) {
                              $$->addNext(AstAlways::newCAssign($<fl>1, outp->cloneTree(false),
...
gateNot<nodep>:
                gateFront variable_lvalue ',' exprList ')'
                        { AstNodeExpr* inp = $4;
                          while (inp->nextp()) inp = VN_AS(inp->nextp(), NodeExpr);
                          $$ = new AstImplicit{$<fl>1, inp->cloneTree(false)};
                          $$->addNext(AstAlways::newCAssign($<fl>1, $2, new AstNot{$<fl>1,
                                                              GRAMMARP->createGatePin(inp->cloneTree(false))}));
                          for (AstNodeExpr* outp = $4; outp->nextp(); outp = VN_CAST(outp->nextp(), NodeExpr)) {
                              $$->addNext(AstAlways::newCAssign($<fl>1, outp->cloneTree(false),
                                                                 new AstNot{$<fl>1,
...
gateBufif0<nodep>:
                gateFront variable_lvalue ',' gatePinExpr ',' gatePinExpr ')'
                        { $$ = new AstImplicit{$<fl>1, $6->cloneTree(false)};
                          ...
                          $$->addNext(AstAlways::newCAssign($<fl>1, $2, rhsp));
                          DEL($1); }
        ;
gateBufif1<nodep>:
                gateFront variable_lvalue ',' gatePinExpr ',' gatePinExpr ')'
