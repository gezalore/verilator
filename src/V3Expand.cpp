...
class SubstUseVisitor final : public VNVisitorConst {
    ...
    SubstVarEntry* findEntryp(AstVarRef* nodep) {
        return reinterpret_cast<SubstVarEntry*>(nodep->varp()->user3p());  // Might be nullptr
    }
    // VISITORS
    void visit(AstVarRef* nodep) override {
        ...
        if (entryp) {
            ...
            if (m_origStep < nodep->varp()->user4()) {
                if (m_ok) {
...
class SubstVisitor final : public VNVisitor {
    ...
    // AstVar::user3p           -> SubstVar* for usage var, 0=not set yet. Only under CFunc.
    // AstVar::user4            -> int step number for last assignment, 0=not set yet
    ...
    SubstVarEntry* getEntryp(AstVarRef* nodep) {
        ...
        if (!varp->user3p()) {
            ...
            varp->user3p(&m_entries.back());
        }
        return varp->user3u().to<SubstVarEntry*>();
    }
    ...
    void visit(AstVarRef* nodep) override {
        ...
        if (nodep->access().isWriteOrRW()) {
            m_assignStep++;
            nodep->varp()->user4(m_assignStep);
            UINFO(9, " ASSIGNstep u2=" << nodep->varp()->user4() << " " << nodep << endl);
        }
