...
private:
    ...
    static AstNode* newWordAssign(AstNodeAssign* placep, uint32_t word, AstNode* lhsp,
                                  AstNode* rhsp) {
        FileLine* const fl = placep->fileline();
        return new AstAssign{fl, new AstWordSel{fl, lhsp->cloneTree(true), new AstConst{fl, word}},
                             rhsp};
    }
    static void addWordAssign(AstNodeAssign* placep, uint32_t word, AstNode* lhsp, AstNode* rhsp) {
        insertBefore(placep, newWordAssign(placep, word, lhsp, rhsp));
    }
    static void addWordAssign(AstNodeAssign* placep, uint32_t word, AstNode* rhsp) {
    ...
    static AstNode* newWordGrabShift(FileLine* fl, int word, AstNode* lhsp, int shift) {
        ...
        if (const uint32_t loffset = VL_BITBIT_E(shift)) {
            AstNode* const lhip = newAstWordSelClone(lhsp, othword - 1);
            const uint32_t nbitsonright = VL_EDATASIZE - loffset;  // bits that end up in lword
            newp = new AstOr{
                fl,
    ...
    static AstNode* newWordSel(FileLine* fl, AstNode* fromp, AstNode* lsbp,
        ...
        if (wordOffset >= fromp->widthWords()) {
    ...
    bool expandLhs(AstNodeAssign* nodep, AstSel* lhsp) {
        ...
        if (VN_IS(lhsp->lsbp(), Const)) {
            ...
            UASSERT_OBJ(lhsp->lsbConst() >= 0, lhsp, "LSB index should be non-negative");
            UASSERT_OBJ(lhsp->msbConst() >= 0, lhsp, "MSB index should be non-negative");
            const uint32_t lsb = lhsp->lsbConst();
            const uint32_t msb = lhsp->msbConst();
            V3Number maskset{nodep, destp->widthMin()};
            for (uint32_t bit = lsb; bit < (msb + 1); bit++) maskset.setBit(bit, 1);
            V3Number maskold{nodep, destp->widthMin()};
            maskold.opNot(maskset);
            if (destwide) {
                UINFO(8, "    ASSIGNSEL(const,wide) " << nodep << endl);
                for (uint32_t w = 0; w < destp->widthWords(); ++w) {
            ...
            if (destwide && lhsp->widthConst() == 1) {
                ...
                for (uint32_t bit = 0; bit < lhsp->widthConst(); bit++) maskwidth.setBit(bit, 1);
    ...
    bool expandWide(AstNodeAssign* nodep, AstConcat* rhsp) {
        ...
        const uint32_t rhsshift = rhsp->rhsp()->widthMin();
        ...
        for (uint32_t w = 0; w < rhsp->widthWords(); ++w) {
            addWordAssign(nodep, w,
    ...
    virtual void visit(AstReplicate* nodep) override {
        ...
        if (nodep->isWide()) {
            ...
            AstNode* lhsp = nodep->lhsp()->unlinkFrBack();
            AstNode* newp;
            const uint32_t lhswidth = lhsp->widthMin();
            if (lhswidth == 1) {
                ...
                for (uint32_t repnum = 1; repnum < times; repnum++) {
                    const uint32_t rhsshift = repnum * lhswidth;
                    newp = new AstOr{fl,
                                     new AstShiftL{fl, lhsp->cloneTree(true),
                                                   new AstConst{fl, rhsshift}, nodep->width()},
                                     newp};
                    newp->dtypeFrom(nodep);  // Unsigned
                }
    ...
    virtual void visit(AstChangeXor* nodep) override {
        ...
        for (uint32_t w = 0; w < nodep->lhsp()->widthWords(); ++w) {
