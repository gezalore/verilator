...
std::unique_ptr<Graph> buildGraph(const LogicByScope& lbs) {
    ...
    const auto addEdge = [&](V3GraphVertex* fromp, V3GraphVertex* top, int weight, bool cuttable) {
        new V3GraphEdge{graphp, fromp, top, weight, cuttable};
    };

    lbs.foreach ([&](AstScope* scopep, AstActive* activep) {
        ...
        for (AstNode* nodep = activep->stmtsp(); nodep; nodep = nodep->nextp()) {
            ...
            LogicVertex* const lvtxp = new LogicVertex{graphp, nodep, scopep};
            ...
            nodep->foreach<AstVarRef>([&](AstVarRef* refp) {
                ...
                // If read, add var -> logic edge TODO: explain the third term in the condition
...
void reportCycles(Graph* graphp, const std::vector<VarVertex*>& cutVertices) {
    for (VarVertex* vvtxp : cutVertices) {
        ...
        // TODO: no clock involved here, fix message
        vscp->v3warn(UNOPTFLAT, "Signal unoptimizable: Feedback to clock or circular logic: "
                                    << vscp->prettyNameQ());
...
LogicByScope breakCycles(AstNetlist* netlistp, LogicByScope& combinationalLogic) {
    ...
    graphp->acyclic(&V3GraphEdge::followAlwaysTrue, /* colorIsSCC: */ true);
